...
interface IStartGameOptional {
  ...
  serverAuthorativeBlockBreaking: boolean,
  currentTick: bigint,
...
export class StartGame extends BatchedPacket<IStartGame> {
  ...
  constructor(p?: IStartGameRequired & Partial<IStartGameOptional>) {
    super(Packets.START_GAME, [
      ...
      { name: 'movementType', parser: DataType.VARINT, resolve: def(0) },
      { name: 'rewindHistorySize', parser: DataType.VARINT, resolve: def(0) },
      { name: 'serverAuthorativeBlockBreaking', parser: DataType.BOOLEAN, resolve: def(false) },
